<?php
  ...
  if($action == "addFaculty"){
    ...
    $password = substr($name,0,4) . substr($dob,0,4);
    ...
    global $conn;
    $sql = "insert into faculty(name,dob,email,contact,address,department,designation,password) values('{$name}','{$dob}','{$email}',{$contact},'{$address}','{$department}','{$designation}','{$encrypted_password}')";
