  ...
  elseif($action == "todaysSchedule"){
    ...
    if($className == ""){
      try {
        ...
        faculty = (select concat(designation,'.',name) from faculty where email = '{$facultyEmail}');";
        $result = mysqli_query($conn,$sql);
        if(mysqli_num_rows($result) > 0){
          $container = "<div class='data'>";  
          $no = 1;
          while($row = mysqli_fetch_array($result)){
            ...
            <span>Faculty</span>:<span id='class_name'>{$row["class"]}</span>
          </div>";
          $no += 1;
          }
          $container .= "</div>";
      ...
      try {
        $sql = "select * from schedule where day ='{$day}' and class = '{$className}' and 
        faculty = (select concat(designation,'.',name) from faculty where email = '{$facultyEmail}');";
        $result = mysqli_query($conn,$sql);
        if(mysqli_num_rows($result) > 0){
          $container = "<div class='data'>";  
